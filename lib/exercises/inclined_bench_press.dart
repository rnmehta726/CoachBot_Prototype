...
class IncBenchPress extends StatefulWidget {
  final List<CameraDescription> cameras;
  final String exercise;

  IncBenchPress(this.cameras, this.exercise);
  @override
  State<StatefulWidget> createState() => _IncBenchPressState();
}

class _IncBenchPressState extends State<IncBenchPress> {
  ...
  Future<void> processImage(InputImage inputImage) async {
    ...
    if (poses.isNotEmpty) {
      ...
      angles['leftShoulder'] = calcAngle(
          ...
          poses[0].landmarks[PoseLandmarkType.leftShoulder]!,
          poses[0].landmarks[PoseLandmarkType.leftElbow]!);
      angles['rightWrist'] = calcAngle(
          poses[0].landmarks[PoseLandmarkType.rightElbow]!,
          poses[0].landmarks[PoseLandmarkType.rightWrist]!,
          poses[0].landmarks[PoseLandmarkType.rightIndex]!);
      angles['leftWrist'] = calcAngle(
          poses[0].landmarks[PoseLandmarkType.leftElbow]!,
          poses[0].landmarks[PoseLandmarkType.leftWrist]!,
          poses[0].landmarks[PoseLandmarkType.leftIndex]!);


      // if(angles['rightShoulder']!>75){
      //   badShoulder++;
      // }
      // down 180-170
      // up 70
      ...
      // if badWrist == 30, save in firebase
      // if badShoulder == 30, save in firebase
    }
