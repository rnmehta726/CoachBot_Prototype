...
class RomDeadlift extends StatefulWidget {
  final List<CameraDescription> cameras;
  final String exercise;

  RomDeadlift(this.cameras, this.exercise);
  @override
  State<StatefulWidget> createState() => _RomDeadliftState();
}

class _RomDeadliftState extends State<RomDeadlift> {
  ...
  Future<void> processImage(InputImage inputImage) async {
    ...
    if (poses.isNotEmpty) {
      ...
      if(angles['rightHip']! < 80){
        bad++;
      }
      // down 180-170
      // up 70
      if (angles['rightHip']! > 170.0) {
        if (!_up) {
          _up = true;
          _reps += 1;
        }
        _good = true;
      } else if (angles['rightHip']! < 95) {
        if (_up) _up = false;
        _good = true;
      } else {
        _good = false;
