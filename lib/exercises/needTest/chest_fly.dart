...
class ChestFly extends StatefulWidget {
  final List<CameraDescription> cameras;
  final String exercise;

  ChestFly(this.cameras, this.exercise);
  @override
  State<StatefulWidget> createState() => _ChestFly();
}

class _ChestFly extends State<ChestFly> {
  ...
  Future<void> processImage(InputImage inputImage) async {
    ...
    if (poses.isNotEmpty) {
      ...
      if(angles['rightElbow']! > 175){
        bad++;
      }
      // down 180-170
      // up 70
      if (angles['rightShoulder']! > 160.0) {
        if (_up) _up = false;
        _good = true;
      } else if (angles['rightShoulder']! < 95) {
        if (!_up) {
          _up = true;
          _reps += 1;
        }
