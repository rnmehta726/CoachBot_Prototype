...
class LatRaise extends StatefulWidget {
  ...
  LatRaise(this.cameras, this.exercise);
  @override
  State<StatefulWidget> createState() => _LatRaiseState();
}

class _LatRaiseState extends State<LatRaise> {
  ...
  Future<void> processImage(InputImage inputImage) async {
    ...
    if (poses.isNotEmpty) {
      ...
      if(angles['rightElbow']!>165){
        bad++;
      }
      // down 180-170
      // up 70
      if (angles['rightShoulder']! > 75.0) {
        if (!_up) {
          _up = true;
          _reps += 1;
        }
        _good = true;
      } else if (angles['rightShoulder']! < 10) {
        if (_up) _up = false;
        _good = true;
      } else {
        _good = false;
