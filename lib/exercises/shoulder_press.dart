...
class ShoulderPress extends StatefulWidget {
  ...
  ShoulderPress(this.cameras, this.exercise);
  @override
  State<StatefulWidget> createState() => _ShoulderPressState();
}
...
class _ShoulderPressState extends State<ShoulderPress> {
  ...
  Future<void> processImage(InputImage inputImage) async {
    ...
    if (poses.isNotEmpty) {
      ...
      // down 180-170
      // up 70
      if(angles['rightShoulder']!<70){
        bad++;
      }

      if (angles['rightElbow']! > 160.0 && angles['leftElbow']! > 160.0) {
        if (!_up) {
          _up = true;
          _reps += 1;
        }
        _good = true;
      } else if (angles['rightElbow']! < 95 && angles['leftElbow']! < 95) {
        if (_up) _up = false;
        _good = true;
      } else {
        _good = false;
