...
class TricepPulldown extends StatefulWidget {
  final List<CameraDescription> cameras;
  final String exercise;

  TricepPulldown(this.cameras, this.exercise);
  @override
  State<StatefulWidget> createState() => _TricepPulldownState();
}

class _TricepPulldownState extends State<TricepPulldown> {
  ...
  Future<void> processImage(InputImage inputImage) async {
    ...
    if (poses.isNotEmpty) {
      ...
      if(angles['rightShoulder']! < 0){
        bad++;
      }
      // down 180-170
      // up 70
      if (angles['rightShoulder']! > 30) {
        bad++;
        _good = false;
      } else if (angles['rightElbow']! > 160.0) {
        if (_up) {
          _up = false;
          _reps += 1;
        }
