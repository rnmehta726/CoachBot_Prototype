...
class LegPress extends StatefulWidget {
  ...
  LegPress(this.cameras, this.exercise);
  @override
  State<StatefulWidget> createState() => _LegPressState();
}
...
class _LegPressState extends State<LegPress> {
  ...
  Future<void> processImage(InputImage inputImage) async {
    ...
    if (poses.isNotEmpty) {
      ...
      if (angles['rightKnee']! > 170.0) {
        bad++;
        _good = false;
      } else if (angles['rightKnee']! > 150.0 && angles['rightKnee']! < 170.0) {
        if (!_up) {
          _reps += 1;
          _up = true;
        }
        _good = true;
      } else if (angles['rightKnee']! < 90) {
        if (_up) _up = false;
        _good = true;
      } else {
        _good = false;
